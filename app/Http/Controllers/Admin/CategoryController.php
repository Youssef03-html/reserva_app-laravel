...
class CategoryController extends Controller
{
    ...
    {
        $categories = Category::all(); // recolleixo tots els events de la BD
    ...
    {
        return view('admin.categories.create'); // redirigeixo al formulari de creació categoria
    }
    ...
    public function store(Request $request)
    {
        $validated = $request->validate([
            'name' => 'required|string|max:255' // validacio del nom de la categoria
        ]);

        Category::create($validated); // creo la categoria
